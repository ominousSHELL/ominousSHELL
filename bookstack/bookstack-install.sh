...
# The directory to install BookStack into BOOKSTACK_DIR="/var/www/bookstack"
BOOKSTACK_DIR="/var/www/bookstack"

# Get the domain from the arguments (Requested later if not set)
DOMAIN="ominousshell.com"
...
function run_package_installs() {
  apt update
  apt install -y git unzip apache2 php8.2 curl php8.2-curl php8.2-mbstring php8.2-ldap \
  php8.2-xml php8.2-zip php8.2-gd php8.2-mysql mariadb-server libapache2-mod-php8.2
}

function run_database_setup() {
  nohup mysqld --user=root > /dev/null 2>&1 & disown && sleep 10
  mysql -u root --execute="CREATE DATABASE ominousshell;"
  mysql -u root --execute="CREATE USER 'bookstack'@'localhost' IDENTIFIED BY '$DB_PASS';"
  mysql -u root --execute="GRANT ALL ON ominousshell.* TO 'bookstack'@'localhost';FLUSH PRIVILEGES;"
}
...
function run_update_bookstack_env() {
  cd "$BOOKSTACK_DIR" || exit
  cp .env.example .env
  sed -i.bak "s@APP_URL=.*\$@APP_URL=http://$DOMAIN:2000@" .env
  sed -i.bak 's/DB_DATABASE=.*$/DB_DATABASE=ominousshell/' .env
...
function run_custom_checks(){
	mkdir -p /run/mysqld/
	sed -i 's/Listen 80/Listen 2000/g' /etc/apache2/ports.conf
}
...
info_msg "[0/9] Installing required system packages... (This may take several minutes)"
run_package_installs

info_msg "[1/9] Running Custom Checks..."
run_custom_checks
