...
async function category_counter(){
    ...
    for (var i=0; i < ID.length; i++){
		...
		var response = await fetch("http://ominousshell.com:2000/database_blog/?blogs_counter=" + search_counter);
	...
	if (data.length > 0){
		...
		for (var i=0; i < data.length; i++){
			...
			id.addEventListener('click', () => {
				window.location.href = `http://ominousshell.com:2000/blogs/${CATEGORY_URL}/${ID}.html`;
			});
...
async function request_blog(element){
	...
	var response = await fetch(`http://ominousshell.com:2000/database_blog/?query=${search_term}`);
