...
async function platform_counter(){
	...
	for (var i=0; i < ID.length; i++){
		...
		var response = await fetch("http://ominousshell.com:2000/database_notes/?platform_counter=" + search_counter);
...
async function category_counter_notes(){
	...
	for (var i=0; i < ID.length; i++){
		...
		var response = await fetch("http://ominousshell.com:2000/database_notes/?notes_counter=" + search_counter);
		var counter = await response.text();
		update_counter_notes(ID[i], counter);
	}
}
	...
	if (data.length > 0){
		...
		for (var i=0; i < data.length; i++){
			...
			id.addEventListener('click', () => {
				window.location.href = `http://ominousshell.com:2000/notes/${CATEGORY}/${PLATFORM}/${ID}.html`;
			});
...
async function request_note(element, platform){
	...
	var response = await fetch(`http://ominousshell.com:2000/database_notes/?query=${search_term_notes}&platform=${platform}`);
